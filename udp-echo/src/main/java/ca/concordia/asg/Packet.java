...
public class Packet implements Comparable<Packet>{
    ...
    public static final int MAX_LEN = MIN_LEN + 1013; //1024
    ...
    public Packet(int type, long sequenceNumber, InetAddress peerAddress, int peerPort, byte[] payload) {
     ...
     // Makes sure that the packet never accepts a payload that is greater than what it can handle
    	if(payload.length > (MAX_LEN - MIN_LEN))
    	{
    		this.payload = new String(payload, UTF_8).substring(0, MAX_LEN - MIN_LEN).getBytes();
    	}
    	else
    	{
    		String strpl = new String(payload, UTF_8);
    		this.payload = strpl.getBytes();   // Doing it this way to avoid shallow copying of bytes.
    	}
    ...
    public Packet(Packet p)
    {
        ...
        // Doing it this way to avoid shallow copy of bytes
        String pl = new String(p.payload, UTF_8);
        this.payload = pl.getBytes();
	...
	public int compareTo(Packet other) {
		...
		return Integer.compare((int)this.getSequenceNumber(), (int)other.getSequenceNumber());
	}
}
